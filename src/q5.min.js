export default function Q5(scope, container) {
  'use strict'
  ...
  function graphics(scope) {
    ...
    $.loadFont = function(url,success,failure){
      ...
      if (typeof url == 'string') {
        ...
        if (url.indexOf("http") !== -1) {
          ...
          sp = ('/font' + Math.floor(Math.random() * 1000000) + '.tff').split('/')
        }
        content = `url('${url}')`
        name = sp[sp.length-1].split(".")[0].replace(" ","");
      } else {
        name = 'font' + Math.floor(Math.random() * 1000000)
        content = url
      }
      let f = new FontFace( name , content);
          f.load().then(
            function(font){
    ...
    function _draw() {
      ...
      ++$.frameCount
      if ($.frameCount % 60 == 0) {
        millisOffset = window.performance.now() - millisStart
    ...
    $.avgRate = function () {
      return $.frameCount / ($.millis() / 1000)
    }
    $.currRate = function () {
      return ($.frameCount - frameOffset) / ($.millis() - millisOffset) * 1000
    }
  }
}
