...
describe("Problem Tests", function () {
  ...
  it("onlyOwner functions are really only Owner", async function () {
    ...
    await expect(problems.updateSolverAddress(addr1.address)).to.not.be
      .reverted;

    await expect(problems.updateMetadataAddress(addr1.address)).to.not.be
      .reverted;

    await expect(problems.updateBodiesAddress(addr1.address)).to.not.be
      .reverted;
  ...
  it("emits 'EthMoved' events when eth is moved", async () => {
    ...
    const balanceOfAddr1Before = await ethers.provider.getBalance(
      addr1.address,
    );
    ...
    expect(balanceOfAddr1After.sub(balanceOfAddr1Before)).to.equal(
      correctPrice,
    );
  });
